...
'''
python main_botiot.py --teacher_model=BinaryLSTM
python main_botiot.py --teacher_model=BinaryLSTMWithAttention
python main_botiot.py --teacher_model=BinaryL3LSTM
python main_botiot.py --teacher_model=BiLSTMWithAttention
python main_botiot.py --teacher_model=BiLSTM2WithAttention
'''
...
if __name__ == '__main__':
    ...
    flow_split = statistics["train num / test num (flow)"]
    ...
    class_weights = compute_class_weight(
        'balanced',
        classes=np.arange(args.labels_num),
        y=y_train
    )
    ...
    args.class_weights = torch.tensor(class_weights, dtype=torch.float).to(gpu_id)

    print(f"Number of classes: {args.labels_num}")
    print(f"Training flow counts: {train_counts}")
    print(f"Class weights (balanced): {class_weights}")
    print(f"Class weights (for PyTorch): {args.class_weights}")
