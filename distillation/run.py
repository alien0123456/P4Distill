...
# Parameter grid
tasks = list(itertools.product(alpha_values, T_values, teacher_model))
...
def run_task(a, T, teacher_name):
    cmd = f"python main_botiot.py --a={a} --T={T} --teacher_model={teacher_name}"
    ...
    for a, T, teacher_name in tasks:
        while True:
            ...
            if len(running_processes) < MAX_CONCURRENT_TASKS:
                proc = run_task(a, T, teacher_name)
