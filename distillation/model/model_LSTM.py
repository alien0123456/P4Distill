...
class LSTM(nn.Module):
    ...
    def forward(self, len_x, ipd_x):
        ...
        evs = self.fc(ebd_cat)
        seq_len = evs.size(1) // self.rnn_in_bits
        rnn_in = evs.view(batch_size, seq_len, self.rnn_in_bits)

        lstm_out, _ = self.lstm(rnn_in)  # lstm_out: [batch_size, seq_len, hidden_size]
