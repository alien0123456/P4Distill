...
class BinaryGRUCell(nn.Module):
    def __init__(self, rnn_in_bits, rnn_hidden_bits, activation_quantizer, num_layers=2):
        super(BinaryGRUCell, self).__init__()
        self.rnn = nn.GRU(
            input_size=rnn_in_bits,
            hidden_size=rnn_hidden_bits,
    ...
    def forward(self, input, time_steps):
        ...
        h0 = torch.zeros(self.num_layers, input.size(0), self.rnn_hidden_bits, device=input.device)

        # Forward pass through GRU
        output, hn = self.rnn(input, h0)

        # Quantize the final hidden state
        final_hidden = self.activation_quantizer(hn[-1])  # Use the last layer's hidden state
        return final_hidden
...
class BinaryRNN(nn.Module):
    def __init__(self, args):
        ...
        self.binary_gru = BinaryGRUCell(self.rnn_in_bits, self.rnn_hidden_bits, self.activation_quantizer, num_layers=2)
    ...
    def forward(self, len_x, ipd_x):
        ...
        # Fully connected layer and quantization
