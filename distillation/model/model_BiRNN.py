...
class StandardBiGRUCell(nn.Module):
    def __init__(self, rnn_in_bits, rnn_hidden_bits):
        super(StandardBiGRUCell, self).__init__()
        self.forward_cell = StandardGRUCell(rnn_in_bits, rnn_hidden_bits)
        self.backward_cell = StandardGRUCell(rnn_in_bits, rnn_hidden_bits)
        self.rnn_in_bits = rnn_in_bits
        self.rnn_hidden_bits = rnn_hidden_bits

    def forward(self, input, time_steps):
        forward_hidden = self.forward_cell(input, time_steps)
        reversed_input = torch.flip(input, dims=[1])
        backward_hidden = self.backward_cell(reversed_input, time_steps)
        return torch.cat((forward_hidden, backward_hidden), dim=1)


class StandardBiRNN(nn.Module):
    def __init__(self, args):
        super(StandardBiRNN, self).__init__()
        # Feature embedding
        ...
        self.bi_gru = StandardBiGRUCell(self.rnn_in_bits, self.rnn_hidden_bits)
    ...
    def forward(self, len_x, ipd_x):
        ...
        rnn_out = self.bi_gru(rnn_in, self.window_size)
