...
from model.Ternary.Ternary_a import Ternary_a
...
class TBiLSTMWithAttention(nn.Module):
    def __init__(self, args):
        super(TBiLSTMWithAttention, self).__init__()
        # Feature embedding
        ...
        self.activation_quantizer = Ternary_a.apply
        
        # input_tensor = torch.tensor([[0.5, -0.3, 1.2], [-0.7, 0.1, -0.9]], requires_grad=True)
        # ternary_output = Ternary_a.apply(input_tensor)
    ...
    def _initialize_weights(self):
        ...
        nn.init.xavier_uniform_(self.len_embedding.weight)
        nn.init.xavier_uniform_(self.ipd_embedding.weight)
    ...
    def forward(self, len_x, ipd_x):
        ...
        len_ebd_tern = self.activation_quantizer(len_ebd)
        ipd_ebd = self.ipd_embedding(ipd_x)
        ipd_ebd_tern = self.activation_quantizer(ipd_ebd)
        ebd_tern_cat = torch.cat((len_ebd_tern, ipd_ebd_tern), dim=-1)
        batch_size = ebd_tern_cat.shape[0]

        evs = self.fc(ebd_tern_cat)
        evs_tern = self.activation_quantizer(evs)

        lstm_in_tern = evs_tern.view(batch_size, -1, self.rnn_in_bits)

        lstm_out, _ = self.lstm(lstm_in_tern)  # lstm_out: [batch_size, seq_len, 2 * hidden_size]
