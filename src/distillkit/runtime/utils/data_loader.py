    ...
    def __init__(self, args, filename):
        ...
        for ins in instances:
            ...
            for i in range(len(len_seq)):
                len_seq[i] = min(len_seq[i], args.pkt_len_vocab_size - 1)
            ...
            for i in range(len(ipd_seq)):
                ipd_seq[i] = min(int(ipd_seq[i] * 1e6) >> 4, args.ipd_vocab_size - 1)
                # ipd_seq[i] = min(ipd_seq[i] * 1e9 // 16384, args.ipd_vocab_size - 1) # unit: 16384 ns, taking into account the precision of tofino operations
                # ipd_seq[i] = min(round(ipd_seq[i] * 10000), args.ipd_vocab_size - 1) # unit: 0.1 ms
                assert ipd_seq[i] >= 0
