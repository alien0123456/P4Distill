...
def batch2segs(args, batch):
    ...
    for i in range(len(batch[0])):
        ...
        if flow_packets < args.seq_window_size:
            raise Exception('Flow packets < window size!!!')
            # continue
        else:
            segs_idx = [idx for idx in range(0, flow_packets - args.seq_window_size + 1)]
            batch_segs_idx = segs_idx

            for idx in batch_segs_idx:
                len_x_batch.append(len_seq[idx: idx + args.seq_window_size])
                ipd_x_batch.append(ipd_seq[idx: idx + args.seq_window_size])
    ...
    if args.cuda_device_id is not None:
        len_x_batch = len_x_batch.cuda(args.cuda_device_id)
        ipd_x_batch = ipd_x_batch.cuda(args.cuda_device_id)
        label_batch = label_batch.cuda(args.cuda_device_id)
...
class STrainer(object):
    ...
    def Svalidate(self, args, test_loader, model):
        ...
        conf_mat_test = np.zeros([args.num_classes, args.num_classes])
        with torch.no_grad():
    ...
    def Strain(self, args, train_loader, test_loader, teacher_model, student_model, optimizer, scheduler):
        ...
        while True:
            ...
            train_total_loss1 = 0
            train_total_loss2 = 0
            conf_mat_train = np.zeros([args.num_classes, args.num_classes])
            ...
            for batch in train_loader:
                ...
                loss, distillation_loss, task_loss = get_loss(teacher_logits, student_logits, label_batch, args.kd_alpha, args.kd_temperature, args.loss_scope)
                ...
                kl_loss = compute_kl_div(teacher_logits, student_logits, T=args.kd_temperature)
